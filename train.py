...
def train(model, data_iterator, optimizer, scheduler, params):
    ...
    for i in t:
        ...
        loss = model(x=batch_data, token_type_ids=None, attention_mask=batch_masks, y=batch_tags)
...
class Net(nn.Module):
    ...
    def forward(self, x, token_type_ids=None, attention_mask=None, y=None):
        ...
        if y is not None:
            ...
            # weight=torch.tensor([1,30]).float().to(self.device)
...
if __name__ == '__main__':
    ...
    # Prepare model
    model = Net(args, params, params.device)
        # BertForTokenClassification.from_pretrained(args.bert_model_dir, num_labels=len(params.tag2idx))
